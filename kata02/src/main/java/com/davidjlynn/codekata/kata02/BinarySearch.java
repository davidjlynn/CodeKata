...
public class BinarySearch {
  ...
  public static int chop(int searchValue, int[] searchList) {
    ...
    return evaluateGroup(
        searchValue, searchList, firstIndex, 0, searchList.length - 1); // Returning failure value.
  }

  /**
   * The comparator, compares values and works out if the item is not in the list.
   *
   * <p>Uses recursion to get through list.
   *
   ...
   */
  private static int evaluateGroup(
      int searchValue, int[] searchList, int currentIndex, int lowerBound, int upperBound) {
    if (searchValue == searchList[currentIndex]) {
      ...
      if (currentIndex == lowerBound) {
        ...
        return evaluateGroup(
            searchValue,
            searchList,
            indexer(lowerBound, currentIndex - 1),
            lowerBound,
            currentIndex - 1);
      }
      ...
      if (currentIndex == upperBound) {
        ...
        return evaluateGroup(
            searchValue,
            searchList,
            indexer(currentIndex + 1, upperBound),
            currentIndex + 1,
            upperBound);
      }
    }
